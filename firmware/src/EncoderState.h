...
class EncoderState {
 ...
 public:
  EncoderState(int spool_diameter_mm, int ticks_per_rev, int initial_length_mm, int initial_position)
  ...
  float getCordLength() {
    return initial_length_mm + current_position * (MY_PI * spool_diameter_mm / ticks_per_rev);
  }

  // Inverse of getCordLength(): cord length (mm) -> encoder tick count.
  int lengthToPosition(float length_mm) {
    return (int)((length_mm - initial_length_mm) / (MY_PI * spool_diameter_mm / ticks_per_rev));
  }
